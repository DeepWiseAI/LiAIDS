"""
The training process of liver tumor classification network, 
...
from metrics import *
from datasets.mp_liver_dataset import create_loader, MultiPhaseLiverDataset, RepeatedDataset
import models
...
# Dataset parameters
parser.add_argument('--dataset', default='mr', type=str, help='ct or mr')
...
parser.add_argument('--crop_size', default=(16, 112, 112), type=int, nargs='+', help='cropped image size.')
...
parser.add_argument('--convert2binary', action='store_true', default=False, 
                    help='convert 7 cls label to 2 cls, (1, 3, 6)->1, (0, 2, 4, 5)->0')
parser.add_argument('--rep_dataset_times', default=1, type=int, help='Repeat training dataset n times')
...
parser.add_argument('--pretrained', default='', type=str, metavar='PATH',
                    help='Load pretrained checkpoint for PATH')
...
parser.add_argument('--num-phases', type=int, default=8,
                    help='number of input phases, default=8')
...
parser.add_argument('--val-epoch', default=250, type=int,
...
parser.add_argument('--report-metrics', default=['auc', 'acc', 'f1', 'recall', 'precision', 'kappa'], 
                    nargs='+', type=str, help='All evaluation metrics')
parser.add_argument('--save-metrics', default=['f1', ],
                    nargs='+', type=str, help='save checkpoints with best metrics')
...
def main():
    ...
    if args.distributed:
        ...
        _logger.info('Training with a single process on 1 GPU(s).')
    assert args.rank >= 0
    ...
    if args.rep_dataset_times > 1:
        dataset_train = RepeatedDataset(args, is_training=True, rep_times=args.rep_dataset_times)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if metric_savers is not None:
                ...
                save_metrics = args.report_metrics if args.save_metrics is None else args.save_metrics
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input)
            if args.aux_loss:
                loss_main = loss_fn(output["main"], target)
                loss_aux = []
                for aux_out in output["aux"]:
                    loss_aux.append(loss_fn(aux_out, target))
                loss = loss_main + sum(loss_aux) / len(loss_aux)
            else:    
...
def compute_metrics(outputs, targets, loss_fn, args):
    ...
    auc = AUC(outputs, targets, num_classes=args.num_classes)
